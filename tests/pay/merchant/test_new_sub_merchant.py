...
@mock_http_response(responses.POST, "/binancepay/openapi/submerchant/add", mock_response, 200)
...
def test_new_sub_merchant_without_merchantName():
    ...
    client = Client(key, secret)
    client.new_sub_merchant.when.called_with(**params).should.throw(ParameterRequiredError)
